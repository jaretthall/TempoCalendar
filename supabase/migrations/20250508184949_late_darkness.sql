...
CREATE POLICY "Allow authenticated users to insert" ON calendar_notes 
  FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "Allow users to update own notes" ON calendar_notes 
  FOR UPDATE USING (auth.uid() = user_id);

-- Create policies for calendar_comments
CREATE POLICY "Allow public read access" ON calendar_comments 
  FOR SELECT USING (true);

CREATE POLICY "Allow authenticated users to insert" ON calendar_comments 
  FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

CREATE POLICY "Allow users to update own comments" ON calendar_comments 
  FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY "Allow users to delete own comments" ON calendar_comments 
